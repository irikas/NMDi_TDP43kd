...
biocPackages <- c("apeglm", "DESeq2","limma", "biomaRt")
...
# Pre-filtering
dds <- dds[rowSums(counts(dds)) >= 110,] # at least 5 reads on average across all samples
...
### Look at DEGs between UPF1/XRN1 and control!
For Y.Y. paper: https://www.biorxiv.org/content/10.1101/2025.06.13.659508
```{r upf1}
setwd("/Users/irika/Library/CloudStorage/OneDrive-JohnsHopkins/WongLing/Scripts/250325_NMDi")
# Load data
load("data/nmdi_all_deseq2.dds.RData")
...
res <- resXU %>% as.data.frame()

## Use Ensembl database to download gene names
...
# Save
# write.csv(res, "output_tables/250905_YY_UPF1XRN1kd_vs_Ctrl_DEGs.csv")
...
colnames(dds@colData)[which(colnames(dds@colData) == "Group1")] = "Treatment"
colnames(dds@colData)[which(colnames(dds@colData) == "Group2")] = "Rep"
dds@colData$NMDi = as.factor(c(rep("shCTR",2),rep("US",2),rep("X",2),rep("XS",2),rep("XU",2)))
design(dds) <- ~ NMDi 
dds$NMDi <- relevel(dds$NMDi, ref = "shCTR")
...
# Create dataframe with logFC and p.adj values for each gene. Add empty cols for CE type, CE name, TDPCE (1-PSI)
dds <- DESeq(dds)
resXU <- lfcShrink(dds, coef="NMDi_XU_vs_shCTR", type="apeglm")
...
# Save
# saveRDS(dds, "output_tables/2501124_YY_UPF1XRN1kd_vs_Ctrl.RDS")
# write.csv(res, "output_tables/2501124_YY_sgTDP_UPF1XRN1kd_vs_Ctrl_DEGs.csv")
