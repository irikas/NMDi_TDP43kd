...
#SBATCH --job-name=eclip_tdp43_K562
...
# Define variables
# ---------------------------
MAIN_DIR="/data/jling2/irika/clipdata/encode_K562_ENCSR720BJU"
EXPERIMENT_NAME="encode_K562_ENCSR720BJU"
...
mkdir -p $SING_DIR $SING_IMAGES $SINGULARITY_TMPDIR $SINGULARITY_CACHEDIR $OUT_DIR $MAIN_DIR/data $OUT_DIR/UMIdedup $OUT_DIR/iCount

# ---------------------------
# Load modules
# ---------------------------
ml load sra-tools/3.0.3 singularity/3.8.7 fastqc/0.11.9
python3 /data/jling2/bin/email/sendemail.py -s "${EXPERIMENT_NAME} clip_tdp43" -c "clip_tdp43 START" -r isinha1@jh.edu
...
for file in *.bam; do \
  if [ ! -e ${file%.bam}_Aligned.sortedByCoord.out.bam ]; then	
...
python3 /data/jling2/bin/email/sendemail.py -s "${EXPERIMENT_NAME} clip_tdp43" -c "clip_tdp43 UMI-tools finished running" -r isinha1@jh.edu
...
for file in *_AlignSortDedup.bam; do \
  if [[ ! -e "${file%_AlignSortDedup.bam}_cDNA_unique.bed" || ! -e "${file%_AlignSortDedup.bam}_cDNA_multiple.bed" || ! -e "${file%_AlignSortDedup.bam}_cDNA_skipped.bam" ]]; then
      singularity exec $SING_IMAGES/icount.sif iCount xlsites $file \
      $OUT_DIR/iCount/${file%_AlignSortDedup.bam}_cDNA_unique.bed  $OUT_DIR/iCount/${file%_AlignSortDedup.bam}_cDNA_multiple.bed $OUT_DIR/iCount/${file%_AlignSortDedup.bam}_cDNA_skipped.bam \
      --group_by start --quant cDNA
  else
    echo "${file} xlsites already complete"
  fi
done
...
python3 /data/jling2/bin/email/sendemail.py -s "${EXPERIMENT_NAME} clip_tdp43" -c "${EXPERIMENT_NAME} clip_tdp43 start iCount " -r isinha1@jh.edu
...
# Ending the run
# ---------------------------
python3 /data/jling2/bin/email/sendemail.py -s "${EXPERIMENT_NAME} clip_tdp43" -c "${EXPERIMENT_NAME} clip_tdp43 iCount finished running" -r isinha1@jh.edu
exit 0
