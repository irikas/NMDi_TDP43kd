...
# TOP 30 NAUC
cryptic_df_high30 <- cryptic_df_high %>% group_by(`Gene Symbol`) %>%
  summarize(max = max(maxTDP)) %>% arrange(desc(max)) %>% top_n(30,wt=max) 

nauc_highCE_mat <- nauc_highCE_mat[which(rownames(nauc_highCE_mat) %in% cryptic_df_high30$`Gene Symbol`),]
