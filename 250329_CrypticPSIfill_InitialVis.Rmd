...
for(i in list.files(sj_wd)){
  ...
  tempdf <- fread(paste0(sj_wd,i),col_names =F,lazy = T) %>% 
...
cryptic_df_fill$`ID #` <- paste0("CE", sprintf("%03d", 1:422))
...
cryptic_df_fill[,6:27] = NA
colnames(cryptic_df_fill)[6:16] = paste0("LPSI_",colnames(junctionCountdf)[2:12])
colnames(cryptic_df_fill)[17:27] = paste0("RPSI_",colnames(junctionCountdf)[2:12])

for(i in 1:nrow(cryptic_df_fill)){
  ### Identify inclusion Counts (skiptic)
  incJunc = skiptic_df_fill$skiptic_junction
  incdf = junctionCountdf[which(junctionCountdf$Coordinates %in% incJunc),] %>% 
    dplyr::select(!Coordinates) %>% apply(2,function(x) sum(x,na.rm=T))
  incdf[is.na(incdf)] = 0
    
  ### Left canonical junction counts
  L_excJunc = unlist(as.list(str_split(cryptic_df_fill$canonical_splice_junction[i],";")))
    excdf = junctionCountdf[which(junctionCountdf$Coordinates %in% excJunc),] %>% 
      dplyr::select(!Coordinates) %>% apply(2,function(x) sum(x,na.rm=T))
    excdf[is.na(excdf)] = 0
