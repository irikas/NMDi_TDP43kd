...
library("BiocManager")
packages <- c("tidyverse","ggplot2","data.table","biomaRt", "pheatmap","viridis")
...
# Samples with high CE inclusion
## Subset to avgPSI > 5 for at least 20% targets
## Subset to CEs within those samples with a max inclusion greater than 20
result_df_TDPneg <- result_df_wide_subset_overall%>%
  mutate(n5 = apply(result_df_wide_subset_overall,
                     MARGIN = 1,
                     function(x) length(which(x > 5)))) %>%
...
colKeep <- (apply(result_df_TDPneg,
                     MARGIN = 2,
                     function(x) length(which(x > 20)))>(0.5*nrow(result_df_TDPneg)))
...
#manual
rowAnn = data.frame(sampleID = samples$sampleID,
                    Detail = c(rep("HeLa SMG6/7KD - UPF1 CLIP",2),
                                  "HeLa SMG6/7KD + SMG7 rescue - UPF1 CLIP",
                                  "MCF-7 mRNA",
                                   rep("MDA-MB231 shTDP43/shSRSF3",2),
                                  rep("hMN siTDP-43",2),
                                  "HeLa NBT",
                                  "DLPFC 3M",
                                  "A-172 + TN Drosha miRNA-seq",
                                  rep("MRC-5 MERS-CoV",5),
                                  "MCF-7 mRNA",
                                  "DLPFC 4M",
                                   "HeLa shUPF1"
                                   ),
                    Condition = c(rep("NMD factor",3),
                                  "control",
                                  rep("TDP-43 KD",4),
                                  "misc",
                                  "control",
                                  ...
                                  "NMD factor")) %>%
  column_to_rownames("sampleID") %>%
  mutate(Condition = factor(Condition, c("control","TDP-43 KD", "NMD factor", "MERS", "misc")))

rowAnn <- rowAnn %>% dplyr:: select("Condition")
